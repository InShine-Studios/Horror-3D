...
using Astralization.Utils.Events;
    ...
    public class Inventory : MonoBehaviour, IInventory
    {
        ...
        public void PickItem(Item item)
        {
            ...
            else
            {
                ...
                item.transform.position = transform.position + ActiveItemYOffset;  // Reposition object to middle body of player
                item.transform.rotation = transform.rotation;  // Reset item rotation
                item.Pick();
        ...
        private void DiscardItem()
        {
            ...
            _activeItem.transform.position -= ActiveItemYOffset + new Vector3(0, transform.position.y, 0);
        ...
        public void ScrollActiveItem(Vector2 scrollVector)
        {
            ...
            int newIdx = MathCalcu.mod(_activeIdx - indexShift, _size);
        ...
        public void UseActiveItem()
        {
            ...
            else
            {
                ...
                if (isUsed)
                {
                    if (_activeItem.UseBehaviourType.HasFlag(ItemHelper.UseBehaviourType.Environmental))
                    {
                        DiscardItem();
                    }
                    else if (_activeItem.UseBehaviourType.HasFlag(ItemHelper.UseBehaviourType.Consumable))
                    {
                        RemoveActiveItem();
                    }
                    else if (_activeItem.UseBehaviourType.HasFlag(ItemHelper.UseBehaviourType.Handheld))
                    {
                        InvokeHudEvent(new UpdateHudLogoEventArgs(_activeIdx, _activeItem.GetHudLogoAnimatorController(), _activeItem.LogoState));
                    }
                }
