...
namespace Astralization.Player
{
    ...
    public class PlayerSpriteAnim : MonoBehaviour
    {
        ...
        public void SetSpriteIdleDirection(Transform _rotatable, Transform _mainCamera)
        {
            ...
            perspectiveAngle = MathCalcu.mod((int)(perspectiveAngle * 180), 360);

            if (perspectiveAngle >= 135 && perspectiveAngle < 225)
            {
                _animator.SetInteger("Direction", (int)PlayerHelper.Direction.Down);
            }
            else if (perspectiveAngle >= 45 && perspectiveAngle < 135)
            {
                _animator.SetInteger("Direction", (int)PlayerHelper.Direction.Left);
            }
            else if (perspectiveAngle >= 315 || perspectiveAngle < 45)
            {
                _animator.SetInteger("Direction", (int)PlayerHelper.Direction.Up);
            }
            else if (perspectiveAngle >= 225 && perspectiveAngle < 315)
            {
                _animator.SetInteger("Direction", (int)PlayerHelper.Direction.Right);
            }
        }
