...
namespace Astralization.Enemy
{
    ...
    public class GhostMovement : MonoBehaviour, IGhostMovement
    {
        ...
        {
            return Mathf.Abs(GeometryCalcu.GetDistance3D(transform.position, _wanderTarget));
        }
        ...
        {
            float shiftX = Randomizer.GetFloat(-target.Radius, target.Radius);
            float shiftZ = Randomizer.GetFloat(-target.Radius, target.Radius);
